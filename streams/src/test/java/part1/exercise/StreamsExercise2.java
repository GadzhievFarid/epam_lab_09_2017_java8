...
public class StreamsExercise2 {
        ...
        Map<String, Person> result = getEmployees().stream()
                .flatMap(employee ->
                        employee.getJobHistory()
                                .stream()
                                .map(jobHistoryEntry ->
                                        new PersonEmployerDuration(employee.getPerson(), jobHistoryEntry.getEmployer(), jobHistoryEntry.getDuration())))
                .collect(Collectors.groupingBy(PersonEmployerDuration::getEmployer,
                        Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparingInt(PersonEmployerDuration::getDuration)),
                                g -> g.get().getPerson())));
    ...
    private static class PersonEmployerDuration {
        ...
        private PersonEmployerDuration(Person person, String position, int duration) {
            this.person = person;
